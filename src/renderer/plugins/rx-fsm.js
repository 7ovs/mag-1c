...
class Transitions {
  constructor (T, A) {
    assert(T, 'transitions not found')
    ...
    for (let s in T) {
      ...
      for (let e in tt) {
        const n = tt[e]
        if (isString(n)) {
          assert(this._stateSet.has(n), `state '${n}' is not defined in transitions`)
        } else {
...
class StateMachine {
  constructor (vm) {
    ...
    const transitions = isFunc(opt.transitions) ? opt.transitions.bind(vm).call() : opt.transitions
    const actions = isFunc(opt.actions) ? opt.actions.bind(vm).call() : opt.actions
    const T = new Transitions(transitions, actions)
